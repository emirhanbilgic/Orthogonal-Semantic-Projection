...
Usage:
  python scripts/benchmark_segmentation.py --mat_file scripts/data/gtsegs_ijcv.mat --methods original,gradcam,chefercam
"""
...
def build_sparse_embedding_v2(
    ...
    if len(parts) > 0:
        ...
        # Filter by cosine similarity and re-collect words to pass back
        if args.dict_include_prompts:
            if len(unique_wnids) > 1 and emb_idx is not None:
                 if emb_idx > 0:
    ...
    # return_indices=True also returns the indices of the selected atoms
