#!/usr/bin/env python3
"""
Compute IMAGE-LEVEL AUROC + mIoU + Accuracy + mAP for all methods on ImageNet-Segmentation.

Supports four dictionary strategies:
  S1 = Classes  (other class embeddings as dictionary)
  S2 = WordNet  (hypernyms + hyponyms + siblings)
  S3 = Gemini   (LLM-generated visual concept dictionary)
  S4 = GPT-OSS  (GPT-generated visual concept dictionary)

Shows results BEFORE (baseline, no OMP) and AFTER (with OMP) for each strategy.

Usage:
    python scripts/compute_image_level_auroc.py --limit 100
    python scripts/compute_image_level_auroc.py --strategies S3 --limit 100
    python scripts/compute_image_level_auroc.py --strategies S1 S3 --methods legrad_clip gradcam_siglip
    python scripts/compute_image_level_auroc.py --methods daam_daam --strategies S3 --limit 50
"""
...
# Hyperparameters per (method, model, strategy)
# Key: (method, model_key)  Value: dict of strategy -> {tau_act, T, tau_cos, [omp_beta]}
HYPERPARAMS = {
    ('legrad', 'clip'): {
        'S1': (0.4, 3, 0.55),   'S2': (0.4, 5, 0.5),   'S3': (0.4, 27, 0.6),   'S4': (0.35, 29, 0.7),
    },
    ('legrad', 'siglip'): {
        'S1': (0.375, 6, 0.5),  'S2': (0.375, 11, 0.55), 'S3': (0.4, 23, 0.65), 'S4': (0.425, 15, 0.7),
    },
    ('chefercam', 'clip'): {
        'S1': (0.15, 4, 0.55),  'S2': (0.125, 5, 0.5),  'S3': (0.175, 4, 0.7),  'S4': (0.175, 10, 0.85),
    },
    ('chefercam', 'siglip'): {
        'S1': (0.1, 6, 0.9),   'S2': (0.1, 5, 0.9),    'S3': (0.1, 4, 0.85),   'S4': (0.125, 19, 0.8),
    },
    ('attentioncam', 'clip'): {
        'S1': (0.1, 17, 0.5),  'S2': (0.1, 6, 0.5),    'S3': (0.1, 20, 0.8),   'S4': (0.1, 28, 1.0),
    },
    ('attentioncam', 'siglip'): {
        'S1': (0.125, 5, 0.55), 'S2': (0.1, 8, 0.55),  'S3': (0.125, 28, 0.7), 'S4': (0.15, 7, 0.75),
    },
    ('gradcam', 'clip'): {
        'S1': (0.1, 5, 0.5),   'S2': (0.1, 17, 0.5),   'S3': (0.1, 26, 0.65),  'S4': (0.125, 32, 0.85),
    },
    ('gradcam', 'siglip'): {
        'S1': (0.5, 20, 0.5),  'S2': (0.5, 16, 0.5),   'S3': (0.475, 26, 0.75), 'S4': (0.45, 23, 0.75),
    },
    ('daam', 'daam'): {
        'S1': (0.225, 17, 0.6), 'S2': (0.325, 20, 0.95), 'S3': (0.4, 24, 0.95), 'S4': (0.4, 19, 0.7),
    },
}

DAAM_OMP_BETA = {'S1': 0.275, 'S2': 0.15, 'S3': 0.1, 'S4': 0.1}

STRATEGY_NAMES = {
    'S1': 'Classes', 'S2': 'WordNet', 'S3': 'Gemini', 'S4': 'GPT-OSS',
}

# S1 = dict_include_prompts only
# S2 = WordNet (hypernyms + hyponyms + siblings)
# S3 = Gemini LLM dictionary (visual_concept_dictionary_445.json)
# S4 = GPT dictionary (visual_concept_dictionary_gpt.json)
...
def build_config(method, model_key, strategy):
    """Build a single evaluation config."""
...
def build_all_configs(strategies=None, methods=None):
    """Build configs for all requested strategy+method combinations."""
...
class KeySpaceOMPProcessor:
    ...
    Copied verbatim from optimize_anti_hallucination.py."""
...
def _concepts_from_json_dict(dictionary, wnid):
    """Extract concept list from a Gemini/GPT-style JSON dictionary."""
    entry = dictionary.get(wnid)
    if entry is None:
        return []
    return (
...
def build_sparse_embedding(text_emb_1x, wnid, target_class_name, config, ctx):
    """Build OMP sparse residual embedding for any dictionary strategy.

    ctx is a dict with: tokenizer, model, device, all_text_embs, cls_idx, unique_wnids,
                        wnid_to_classname, gemini_dict, gpt_dict
    """
    ...
    if strategy == 'S1':
        # S1: Other class prompts as dictionary
        all_embs = ctx['all_text_embs']
        cls_idx = ctx['cls_idx']
        if cls_idx > 0:
            parts.append(all_embs[:cls_idx])
        if cls_idx + 1 < all_embs.shape[0]:
            parts.append(all_embs[cls_idx + 1:])

    elif strategy == 'S2':
        # S2: WordNet neighbors (hypernyms + hyponyms + siblings)
        ...
        # S3: Gemini (LLM) dictionary
        concepts = _concepts_from_json_dict(ctx['gemini_dict'], wnid)
        if concepts:
        ...
        # S4: GPT-OSS dictionary
        concepts = _concepts_from_json_dict(ctx['gpt_dict'], wnid)
        if concepts:
            prompts = [f"a photo of a {c}." for c in concepts]
            c_tok = tokenizer(prompts).to(device)
            with torch.no_grad():
...
def get_raw_heatmap(method, model, img_t, text_emb_1x,
    ...
    """Compute a raw heatmap for the given method."""
    if method == 'legrad':
...
def normalize_heatmap(heatmap, method, H_gt, W_gt):
    """Resize and normalize heatmap to [0, 1]."""
...
def compute_seg_metrics(heatmap_norm, gt_tensor, threshold):
    """Compute mIoU/Acc/mAP metrics for a single heatmap against GT."""
...
def get_threshold(method, heatmap_norm, sparse_threshold, use_omp):
    ...
    Matches optimize_anti_hallucination.py threshold logic."""
    if method == 'legrad':
...
def build_daam_competing_concepts(wnid, target_class_name, config, ctx):
    """Build competing concept list for DAAM for any dictionary strategy."""
    strategy = config['dict_strategy']
    atoms = config['atoms']
    ...
    if strategy == 'S1':
        # Other class names
        for w in ctx['unique_wnids']:
            cn = ctx['wnid_to_classname'][w]
            if cn.lower() != target_class_name.lower():
                all_competing.append(cn)
        ...
        all_competing.extend(_concepts_from_json_dict(ctx['gemini_dict'], wnid))

    elif strategy == 'S4':
        all_competing.extend(_concepts_from_json_dict(ctx['gpt_dict'], wnid))
...
def aggregate_metrics(results_dict):
    """Compute global mIoU, pixel accuracy, and mAP from collected results."""
...
def new_results_dict():
    return {
        ...
        'auroc_mean': [],
        'auroc_max': [],
        'auroc_energy': [],  # sum of activations above threshold
    }


# ── Main evaluation ───────────────────────────────────────────────────────

def evaluate_method(config_name, config, mat_file, class_index_path, image_size,
                    limit, num_negatives, seed, gemini_dict_path, gpt_dict_path, device):
    """Evaluate a single method: both baseline (no OMP) and with OMP."""
    ...
    gpt_dict = {}
    if strategy == 'S3':
        path = gemini_dict_path or os.path.join(scripts_dir, 'visual_concept_dictionary_445.json')
        gemini_dict = load_json_dictionary(path)
    elif strategy == 'S4':
        path = gpt_dict_path or os.path.join(scripts_dir, 'visual_concept_dictionary_gpt.json')
    ...
    wnid_to_label = build_wnid_to_label_map(load_imagenet_class_index(class_index_path))
    ...
    actual_limit = min(limit, num_images) if limit > 0 else num_images
    ...
    for wnid in unique_wnids:
        ...
        if label:
            class_label = label[1] if isinstance(label, (list, tuple)) else label
        else:
    ...
    # Shared context passed to build functions
    ctx = {
        'tokenizer': tokenizer, 'model': model, 'device': device,
        'all_text_embs': all_text_embs, 'unique_wnids': unique_wnids,
        'wnid_to_classname': wnid_to_classname,
        'gemini_dict': gemini_dict, 'gpt_dict': gpt_dict,
    }

    # Accumulators: baseline (correct/wrong) and omp (correct/wrong)
    ...
    for idx in tqdm(range(actual_limit), desc=f"  {config_name}"):
        ...
        try:
            ...
            # Sample negatives (same for baseline and OMP to keep comparison fair)
            all_indices = list(range(len(unique_wnids)))
            neg_pool = [i for i in all_indices if i != cls_idx]
            neg_indices = rng.sample(neg_pool, min(num_negatives, len(neg_pool)))

            # ── Helper to process one heatmap ──
            def process_heatmap(heatmap_raw, results_dict, auroc_label, threshold):
                ...
                # Use RAW (pre-normalization) heatmap for AUROC scores.
                # Normalized heatmaps always have max=1.0, destroying discriminative signal.
            ...
            # ════════════════════════════════════════
            # BASELINE (no OMP): use raw text embedding
            # ════════════════════════════════════════
            ...
            if method == 'daam':
                hm_base_c = get_raw_heatmap(method, model, img_t, None,
                                            ...
                                            target_class_name=class_name, competing_concepts=[])
            else:
                hm_base_c = get_raw_heatmap(method, model, img_t, original_1x)
            ...
            for neg_idx in neg_indices:
                ...
                if method == 'daam':
                    hm_base_w = get_raw_heatmap(method, model, img_t, None,
                                                daam_segmenter=daam_segmenter, base_img=base_img,
                                                target_class_name=neg_class_name, competing_concepts=[])
                else:
                    hm_base_w = get_raw_heatmap(method, model, img_t, neg_emb)
            ...
            # ════════════════════════════════════════
            # OMP (with selected dictionary strategy)
            # ════════════════════════════════════════
            ...
            # Correct prompt — OMP
            if method == 'daam':
                competing = build_daam_competing_concepts(wnid, class_name, config, ctx)
                hm_omp_c = get_raw_heatmap(method, model, img_t, None,
                                           daam_segmenter=daam_segmenter, base_img=base_img,
                                           target_class_name=class_name, competing_concepts=competing,
                                           omp_beta=config.get('omp_beta', 1.0))
            else:
                sparse_c = build_sparse_embedding(original_1x, wnid, class_name, config, ctx)
            ...
            for neg_idx in neg_indices:
                ...
                if method == 'daam':
                    ...
                    hm_omp_w = get_raw_heatmap(method, model, img_t, None,
                                               daam_segmenter=daam_segmenter, base_img=base_img,
                                               target_class_name=neg_class_name,
                                               competing_concepts=wrong_competing,
                                               omp_beta=config.get('omp_beta', 1.0))
                else:
                    ctx['cls_idx'] = neg_idx
                    sparse_w = build_sparse_embedding(neg_emb, neg_wnid, neg_class_name, config, ctx)
    ...
    # Image-level AUROC with multiple score types
...
def main():
    parser = argparse.ArgumentParser(
        description='Compute image-level AUROC + segmentation metrics (all dictionary strategies)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""Examples:
  # All strategies, all methods, 100 images
  python scripts/compute_image_level_auroc.py --limit 100

  # Only S3 (Gemini) strategy
  python scripts/compute_image_level_auroc.py --strategies S3 --limit 100

  # Only LeGrad CLIP, all strategies
  python scripts/compute_image_level_auroc.py --methods legrad_clip --limit 100

  # S1 + S3, only LeGrad
  python scripts/compute_image_level_auroc.py --strategies S1 S3 --methods legrad_clip legrad_siglip
""")
    default_mat = os.path.join(scripts_dir, 'data', 'gtsegs_ijcv.mat')
    default_class_idx = os.path.join(project_root, 'resources', 'imagenet_class_index.json')
    parser.add_argument('--mat_file', type=str, default=default_mat)
    parser.add_argument('--class_index_path', type=str, default=default_class_idx)
    parser.add_argument('--gemini_dict_path', type=str, default=None,
                        help='Path to Gemini/LLM dictionary (default: scripts/visual_concept_dictionary_445.json)')
    parser.add_argument('--gpt_dict_path', type=str, default=None,
                        help='Path to GPT dictionary (default: scripts/visual_concept_dictionary_gpt.json)')
    ...
    for config_name, config in configs.items():
        result = evaluate_method(
            ...
            class_index_path=args.class_index_path,
    ...
    W = 115
    print(f"\n\n{'='*W}")
    print("SUMMARY: BASELINE vs OMP  (all dictionary strategies)")
    print(f"{'='*W}")
